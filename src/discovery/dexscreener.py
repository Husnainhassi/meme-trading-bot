...
# TOKEN_PAIRS_URL = "https://api.dexscreener.com/latest/dex/pairs"
TOKEN_PAIRS_URL = "https://api.dexscreener.com/token-pairs/v1"

def fetch_boosted_tokens():
    boosted_api_res = requests.get(BOOSTS_URL)
    boosted_api_res.raise_for_status()  # Raise an error for bad responses
...
def discover_new_pairs() -> list[dict]:
    """
    Discover new token pairs based on boosted tokens on Solana.
    
    Returns:
        Combined list of pair dictionaries for each boosted token,
